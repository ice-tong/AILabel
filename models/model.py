...
from .trex_encoder import DEFAULT_MIN_PARAMS_TO_WRAP, TrexEncoder

logger = logging.getLogger(__name__)


@register_model("ailabel")
class AILabelModel(FairseqEncoderModel):
    @classmethod
    def hub_models(cls):
        return {
            "trex.base": "",
    ...
    def register_maskvar_head(
        ...
        self.classification_heads[name] = TrexLMClassificationHead(
            self.args.encoder_embed_dim, len(self.task.source_dictionary[configs.var_label_field]),
...
class Encoder(FairseqEncoder):
    ...
    def build_encoder(self, args, dictionary, embed_tokens, embed_bytes, **kwargs):
        encoder = TrexEncoder(args, dictionary, embed_tokens, embed_bytes, **kwargs)
...
@register_model_architecture("ailabel", "ailabel")
...
@register_model_architecture("ailabel", "ailabel_prenorm")
...
@register_model_architecture("ailabel", "ailabel_base")
def trex_base_architecture(args):
    base_architecture(args)


@register_model_architecture("ailabel", "ailabel_large")
...
@register_model_architecture("ailabel", "ailabel_xlm")
